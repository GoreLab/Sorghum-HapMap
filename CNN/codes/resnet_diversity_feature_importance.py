...
def DeepEvolution1D(input_shape):
...
def train_DeepEvolution1D(y_train, x_train, y_dev, x_dev, epochs):
    # Define the model:
    model = DeepEvolution1D(input_shape = (x_train.shape[1], 1))
    # Compile the model:
...
columns_df = ['removed_feature', 'set', 'rep', 'rmse', 'accuracy', 'r2']
...
for r in range(n_rep):
    for f in feat_list:
        ...
        # Standarlize data frame:
        trn_std_y, scaler_trn_y = process_input(raw_input = trn_y)
        trn_std_x, scaler_trn_x = process_input(raw_input = trn_x)
        dev_std_y, scaler_dev_y = process_input(raw_input = dev_y)
        dev_std_x, scaler_dev_x = process_input(raw_input = dev_x)
        tst_std_y, scaler_tst_y = process_input(raw_input = tst_y)
        ...
        # Train the model:
        model = train_DeepEvolution1D(y_train = trn_std_y,
                                      x_train = trn_std_x,
                                      y_dev = dev_std_y,
                                      x_dev = dev_std_x,
                                      epochs = epochs_)
        # Get prediction:
        trn_y_hat =  scaler_trn_y.inverse_transform(model.predict(trn_std_x))
        dev_y_pred = scaler_dev_y.inverse_transform(model.predict(dev_std_x))
        tst_y_pred = scaler_tst_y.inverse_transform(model.predict(tst_std_x))
        # Compute metrics and storing information from the current run:
        trn_out = pd.DataFrame([[f, 'trn', r,
                                 rmse(trn_y, trn_y_hat),
                                 pearsonr(trn_y, trn_y_hat)[0][0],
                                 r2_score(trn_y, trn_y_hat)]], columns=columns_df)
        dev_out = pd.DataFrame([[f, 'dev', r,
                                 rmse(dev_y, dev_y_pred),
                                 pearsonr(dev_y, dev_y_pred)[0][0],
                                 r2_score(dev_y, dev_y_pred)]], columns=columns_df)
        tst_out = pd.DataFrame([[f, 'tst', r,
                                 rmse(tst_y, tst_y_pred),
                                 pearsonr(tst_y, tst_y_pred)[0][0],
                                 r2_score(tst_y, tst_y_pred)]], columns=columns_df)
        if (trn_out.accuracy[0] < 0.0001) or np.isnan(trn_out.accuracy[0]):
            continue
...
feat_metrics_table.groupby(['removed_feature'], as_index=False)['accuracy'].mean()
feat_metrics_table.groupby(['removed_feature'], as_index=False)['r2'].mean()

# Set directory:
os.chdir(dir_proj_ + "output")

# Save pickler:
with open('diversity_feat_selection_out.pkl', 'wb') as f:
  pickle.dump(feat_metrics_table, f, protocol=-1)

# Save on csv format:
feat_metrics_table.to_csv('diversity_feat_selection_out.csv', header=True, index=False)
...
with sns.plotting_context(font_scale=1):
    ax = sns.boxplot(x="removed_feature", y="accuracy", hue="Sets",
...
ax.figure.savefig('diversity_feat_selection_out.png', dpi=350)

# Save the figure pdf format:
ax.figure.savefig('diversity_feat_selection_out.pdf', dpi=350)
